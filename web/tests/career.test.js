...
test('career controls connect chapters, seek, reading pause, expansion and re-entry', t => {
  ...
  const down=new Event('pointerdown'); Object.assign(down,{button:0,pointerId:1}); seek.dispatchEvent(down);
  assert.equal(query('#career-play').attributes['aria-label'],'再生');
  seek.dispatchEvent(new Event('pointerup'));
  assert.equal(query('#career-play').attributes['aria-label'],'一時停止');
  ...
  seek.dispatchEvent(down); seek.dispatchEvent(new Event('pointerup'));
  assert.equal(query('#career-play').attributes['aria-label'],'再生');
  ...
  query('#career-play').click();
  seek.dispatchEvent(down); seek.value=seek.max; seek.dispatchEvent(new Event('input')); seek.dispatchEvent(new Event('pointerup'));
  assert.equal(Number(seek.value),seek.max);
  assert.equal(query('#career-play').attributes['aria-label'],'最初から再生');
