...
import { framingDistance, orbitDirection } from './avatar-framing.js';
...
export async function createAvatarScene({ canvas, signal }) {
  ...
  try {
    ...
    const points=[];
    for(const x of [box.min.x,box.max.x]) for(const y of [box.min.y,box.max.y]) for(const z of [box.min.z,box.max.z]) points.push([x-face.x,y-face.y,z-face.z]);
    let distance = 1, viewX = 0, viewY = 0;
    const orbit = () => {
      const direction=orbitDirection(viewX,viewY);
      camera.position.set(face.x+distance*direction[0],face.y+distance*direction[1],face.z+distance*direction[2]);
