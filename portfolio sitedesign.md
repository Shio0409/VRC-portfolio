...
初回選択後の共通Sound切り替えUIは、通常時は丸いボタンとスピーカーアイコンだけを表示します。Hover時に横へ伸びるアニメーションで`SOUND ON / OFF`を表示し、離れると円に戻ります。スピーカーの波線・ミュート記号で状態を示します。キーボードフォーカスでも文言を表示し、タッチでは円形ボタンをタップして切り替えます。
...
- 基本的に作成・修正作業が完了したら、必要な確認後にコミットし、自動でpushする。作業ごとのpush承認は不要とする。ユーザーが個別に調査のみ・push不要等を指示した場合はその指示を優先する。
