...
- スマートフォンは横向きで閲覧し、PCと同じ情報・要素の配置を維持する。今後のTOP / CAREER / WORKS / CONTACTにも適用する。
- 縦向きのスマートフォンには端末を横にする案内を表示し、背後のサイト操作を無効にする。通常のブラウザから画面の向きを強制する実装は行わない。
- 狭いタッチ画面（主ポインターがcoarse、幅767px以下、縦長）を検知する。PCのウィンドウが縦長という理由だけでは案内を表示しない。
- 共通の横画面レイアウトを`web/src/viewport.css`で管理する。四隅の共通UIと中央コンテンツを維持し、画面の高さに応じて画像・余白・文字サイズを調整する。スマホ専用の縦積みや情報の非表示は行わない。
- 通常の操作ボタンは44px以上を確保する。全体を画像のように一括縮小せず、拡大表示も禁止しない。568px未満の幅や極端に低い画面では必要なスクロールを許可し、内容を切り捨てない。
- セーフエリアとブラウザUIを考慮する。回転で画面の状態・Sound設定を初期化しない。縦向きで読込が完了した場合、横向きに戻ると現在の画面を表示する。
- 横向き案内と背後の操作制御は`web/src/viewport.js`で共通化する。負荷調整は画質や装飾粒子数で行う。
...
| Loading | 画像とTOPアバターの準備完了・最低2秒を待つ。SKIP時は先読みを継続し、未準備なら状態文と再試行を表示する |
...
- 背景画像を使わず、個々の光の粒子が下から上へゆっくり動くCSS表現。PCでは16個、主ポインターがcoarseのタッチ端末では10個を表示する。
...
- PC / スマートフォンで共通の横画面レイアウト、縦向き案内、キーボード操作、動きを減らす設定への対応。
...
## アバターの固定表情（2026-09-13追加）

TOP用アバターのBodyブレンドシェイプ`eye_pupil_OFF`は、常に100で固定する（glTF / Three.jsでは1）。初期表示、表情変更、アニメーション再生、リセットのいずれでも変更しない。書き出し・Web表示の両方で適用する。その他の表情、初期ポーズ、採用アニメーションは引き続き個別に決める。

## TOPレイアウト実装状況（2026-09-14）

01 TOPの3列レイアウト、初期表示で開くSkill Menu、USER/3D/DEV/DESIGN/MUSIC/MANAGEの6タブと仕様記載のスキル内容、開閉・再表示・キーボード操作を実装。中央は3Dアバターを接続する領域として確保し、GLBのTOP統合は次工程とする。プロフィールアイコンは実画像未指定のため汎用アイコンを仮使用する。会話全文・ポーズ・アニメーション・Navigationの最終ビジュアルは確定していない。未実装セクションへのNavigationは準備中として無効表示する。

### 後続：TOPへの3D接続

TOP中央へThree.jsのGLB描画を接続。`eye_pupil_OFF`は100固定を継続し、アバター選択でSkill Menuを開く。現段階は元モデルのTポーズ・ギターあり・Animationなしの暫定表示で、採用ポーズを決定したものではない。テクスチャは確認済みの1Kを維持する。

Loading中に3Dを先読みし、テクスチャ・シェーダーの準備完了を待つ。ページ内の画面往復とENTRYへの帰還でも保持し、非表示中は描画を停止する。SKIPしても先読みは継続し、未準備時は状態文、失敗時は再試行を表示する。低性能端末向けのFallbackは引き続き未確定。


## TOP表示の実装更新（2026-09-14）

- TOP背景は画像生成による星空・水辺・発光植物の環境画像を使用。文字・UI・アバターは別レイヤー。Loading背景は従来のパーティクル実装を継続。
- 会話は以下の仮文をユーザー承認の上で実装。最終会話全文としての確定ではない。
  1. こんにちは、sio0409です。
  2. 企画・制作・技術・運営を横断して、アイデアを動く形にしています。
  3. このサイトでは、スキルやこれまでの経験、制作物をご紹介します。
  4. どうぞ、ゆっくり見ていってください。
- 文字送り中のクリックで全文表示、全文表示後のクリックで次へ。最後のセリフからは最初に戻って再読できる。アバター音声なし。文字送り中の名前プレートを発光させる。
- 会話枠とSkill Menuをコンセプトのガラス表現に近づける。カテゴリ変更は引き続き下部6タブのみ。アバターのポーズ・アニメーションは未選定。


## CAREER初期実装の追記

確定原稿の9項目をHTMLプレゼンテーション化。TOPとのNavigation、自動再生、再生・一時停止、シーク、年号ジャンプ、前後の項目、再生時間、拡大表示を接続。PCとスマホ横画面で左Navigation・右Playerの配置を維持する。

初回実装の12秒／項目は調整用の仮値であり、最終再生時間や同年内の詳細順序を確定するものではない。写真と音素材は未用意。Volume/Muteは素材接続まで無効表示。教室長代理の詳細本文等と個人オンラインゲーム開発の項目は引き続き保留。Portal演出や最終背景は後続工程。


## TOP視点・ガラスUIの追加指定

- アバターの足元と背後の装飾円を削除する。
- 掲載内容は変更せず、コンセプトアートのガラスUIに近づける。
- カーソル位置に合わせ、アバターを見るカメラを全方向最大約30度動かす。モデルの初期ポーズの決定とは別の演出。
- 背景を拡大して余白を作り、カメラに連動して背景を動かす視差表現を入れる。
- 今後Unity操作は変更後の `isuzu_unity` MCPを使用する。


### 顔中心の初期カメラ角度

TOPカメラの中心をアバターの顔へ変更し、初期位置を顔から約30度上方の見下ろしにする。カーソル追従はこの初期角度を基準とする。ガラスUIは透け感を強め、表面の光沢を追加する。掲載内容は変更しない。


TOPのカーソル追従方向はユーザー指定により上下左右とも従来と逆方向へ変更。顔中心と上方30度の初期角度は維持する。


### Portal初期接続（2026-09-15）

TOP / CAREERのNavigation移動に0.7秒の短いPortal通過表現を接続。画面と操作は即時切替、演出は入力を妨げない。スクロール接近演出とPortal音は後続工程。確定済みコンテンツは変更しない。
...
### 背景パース・ガラスUIの調整（2026-09-15）

TOP背景の移動方向をカメラの視点に揃える。画像を拡大して余白を確保し、近い側が大きく遠い側が小さく見える軽いパース変形を加える。既存の顔中心・初期俯角30度・視点範囲は維持する。TOP / CAREER / CONTACTのガラス面は透明度を上げ、ぼかしを抑え、縁の光沢・局所的な反射で厚みを表現する。コンセプトを参考にUIの装飾を調整し、掲載内容・未確定項目は変更しない。


### TOPの全身表示と構図調整（2026-09-15）

顔中心の注視点と初期俯角30度を維持し、全身の境界と視点範囲からカメラ距離を計算する。足先がcanvasで切れないよう描画領域を中央ステージ全体に広げ、セリフ・名前は重ねて配置する。コンセプトを参考に吹き出し・Navigation・スキルパネルの配置と装飾を調整する。未確定のポーズや掲載内容は決めない。


### CAREER / CONTACTの室内背景（2026-09-16）

両コンセプトの夜のラウンジを参考に背景素材を生成し、CAREERとCONTACTで共用する。人物・文字・UIは背景へ焼き込まない。経歴やプロフィールの実素材とは区別する。公開画像は約120KBのWebPとし、対象画面で遅延読み込みする。TOPとLoadingの背景は維持する。

### TOPアバター保持・視点追従の更新

アバターはLoading中に準備し、ページを開いている間は保持する。TOPへ戻るたびの読み込みは行わない。視点はカーソルへ即座に追随せず、時定数420msで緩やかに追従する。背景とガラス反射も同じ追従に合わせる。ポーズ・アニメーションの選定は保留を維持する。
